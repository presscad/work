...
CXsrqCardPage::CXsrqCardPage(void)
{
}
...
void CXsrqCardPage::HandleSelected(TNotifyUI& msg)
{
	
}
void CXsrqCardPage::HandleClick(TNotifyUI& msg)
{
	...
	else
	{
		...
		if (strBtnName == "XSRQ_cardnextpage")
		{
			...
			{ 
				...
				{
					if (nSendPage == nToPage)
						g_SockBuss.SendGetKhXsrqCard(2,m_strKuid,m_strdate,nSendPage,3,0);
					else g_SockBuss.SendGetKhXsrqCard(2,m_strKuid,m_strdate,nSendPage,2,atoi(m_id.strLast.c_str()));
				}
		...
		if (strBtnName == "XSRQ_cardprepage")
		{
			...
			else*/
			{
				...
				else
				{
					if (nPre==1)
					{
						g_SockBuss.SendGetKhXsrqCard(2,m_strKuid,m_strdate,nPre);
					}
					else
						g_SockBuss.SendGetKhXsrqCard(2,m_strKuid,m_strdate,nPre,1,atoi(m_id.strFirst.c_str()));
				}
		...
		else if (strBtnName == "XSRQ_cardlastpage")
		{
			...
			{
				g_SockBuss.SendGetKhXsrqCard(1,m_strKuid,m_strdate,nPage,3,0);
			}
		...
		else if ( strBtnName== "XSRQ_cardfirstpage")
		{
			...
			pCardList->RemoveAll();
				...
				g_SockBuss.SendGetKhXsrqCard(1,m_strKuid,m_strdate,1);
...
void CXsrqCardPage::HandleHeaderClick(TNotifyUI& msg)
{
	//return;//暂时屏蔽此功能
	//CListUI* pList= NULL;
...
void CXsrqCardPage::HandleLink(TNotifyUI& msg)
{
...
void CXsrqCardPage::InitInfo(string strJL,string strdate,EN_CARDS_TYPE type)
{
	...
	CListUI* pList =static_cast<CListUI*>(m_pm.FindControl(_T("XSRQ_cardquerylist")));
	pList->SetItemFixedHeight(30);
	pList->RemoveAll();
	CLabelUI* pKehumin =static_cast<CLabelUI*>(m_pm.FindControl(_T("XSRQ_khm_label")));	
	string strKehuShow ="经理名:";
	if (type== en_kehu)
	{
		strKehuShow="客户名：";
	}
...
LRESULT  CXsrqCardPage::OnKehudatecardmsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
...
LRESULT CXsrqCardPage::OnTFJimsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
	object* pObj = (object*)lParam;
	int cmd = pObj->as<int>();
	pObj ++;
	...
	if (nsubcmd == DN_DISABLE  || nsubcmd == DN_ABLE)
	{
		...
		CListUI* pCardList =static_cast<CListUI*>(m_pm.FindControl(_T("XSRQ_cardTFJlist")));
		//pCardList->RemoveAll();
		bool bFind = false;
...
LRESULT  CXsrqCardPage::OnUpdatestatemsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
