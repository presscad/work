...
class CXsrqCardPage: public CWindowWnd, public INotifyUI
{
public:
	CXsrqCardPage(void);
	~CXsrqCardPage(void);
	...
	void ShowhideTFj()
	{
		CButtonUI* pTingji = static_cast<CButtonUI*>(m_pm.FindControl(_T("XSRQ_cardfujibtn")));
		CButtonUI* pFuji = static_cast<CButtonUI*>(m_pm.FindControl(_T("XSRQ_cardfujibtn")));
		if (g_strUserType == "2")
		{
	...
	LRESULT OnCreate(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
	{
		...
		CControlUI* pRoot = builder.Create(_T("经理日期卡.xml"), (UINT)0,  &cb, &m_pm);
	...
	LRESULT HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_DATECARD_MSG:lRes = OnKehudatecardmsg(uMsg, wParam, lParam, bHandled); break;
		case WM_TINGFUJI_MSG:lRes = OnTFJimsg(uMsg, wParam, lParam, bHandled); break;
...
private:
	...
	LRESULT  OnKehudatecardmsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
	LRESULT  OnTFJimsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
	LRESULT  OnUpdatestatemsg(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled);
	public:
void InitInfo(string strJL,string strdate, EN_CARDS_TYPE enType= en_kehujingli);
void SetKhid(string id)
{
	m_strKuid = id;
}
public:
